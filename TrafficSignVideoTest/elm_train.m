function [TrainingTime,TrainingAccuracy] = elm_train(TrainingData_File, Elm_Type, NumberofHiddenNeurons, ActivationFunction,isLoad)
...
% isLoad                - isLoad=1,load data from mat,isLoad=0, read data
%                         from txt
% Output: 
...
%signsHOGFeatures=load(TrainingData_File);
if isLoad==0
    signsHOGFeatures=GTSRBImgDataRead(TrainingData_File);
    save('signsHOGFeatures.mat','signsHOGFeatures');
else
    tmpHOGFeatures=load('signsHOGFeatures.mat');
    signsHOGFeatures=tmpHOGFeatures.signsHOGFeatures;
end

T=signsHOGFeatures(:,1)';  %class Id
P=signsHOGFeatures(:,2:size(signsHOGFeatures,2))';  %HOG feature
clear signsHOGFeatures;                                   %   Release raw training data array
clear tmpTraindata;
